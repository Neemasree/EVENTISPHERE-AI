...
import { getParkingAnalysis } from '../services/api';
    ...
    <motion.div whileHover={{ y: -4, scale: 1.015 }} transition={{ type: 'spring', stiffness: 300 }}
      ...
      <div className="flex items-center gap-2">
        ...
        <motion.button whileTap={{ scale: 0.94 }} onClick={() => onEnter(count)} disabled={isFull}
          ...
          style={isFull
            ? { background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)', color: 'rgba(255,255,255,0.2)', cursor: 'not-allowed' }
...
export default function ParkingPage() {
  const [zones, setZones] = useState<{ A: Zone; B: Zone }>({
    A: { id: 'A', occupied: 420, capacity: 500 },
    B: { id: 'B', occupied: 180, capacity: 500 },
  });
  ...
  // Poll Flask /parking/analyse — single source of truth
  useEffect(() => {
    const sync = () =>
      getParkingAnalysis()
        .then((data: any) => {
          ...
          if (Array.isArray(data?.zones)) {
            const a = data.zones.find((z: any) => z.id === 'parking_a');
            const b = data.zones.find((z: any) => z.id === 'parking_b');
            if (a) setZones(prev => ({ ...prev, A: { id: 'A', occupied: a.currentCrowd, capacity: a.maxCapacity } }));
            if (b) setZones(prev => ({ ...prev, B: { id: 'B', occupied: b.currentCrowd, capacity: b.maxCapacity } }));
          }
        })
        .catch(() => setConn(false));
    sync();
  ...
  const totalCap   = zones.A.capacity + zones.B.capacity;
  ...
  return (
    <div className="space-y-5 max-w-[1400px] mx-auto pb-24">
      <div className="flex items-start justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[11px] font-semibold"
            style={conn
              ? { background: 'rgba(0,245,160,0.08)', border: '1px solid rgba(0,245,160,0.2)', color: '#00f5a0' }
            ...
            <span className="ml-1">{conn ? 'Flask Connected' : 'Local Mode'}</span>
          </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        ] as [string, number, string, string][]).map(([label, val, suf, col], i) => (
      ...
      <motion.div key={ai.text} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}
        ...
        <div className="flex items-center gap-1.5 flex-shrink-0">
          ...
          <motion.button whileTap={{ scale: 0.93 }} onClick={() => setAutoAn(v => !v)}
            ...
            style={autoAn
              ? { background: 'rgba(0,245,160,0.1)', border: '1px solid rgba(0,245,160,0.25)', color: '#00f5a0' }
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          ...
          <p className="text-[10px] text-white/25 font-mono">Drag to rotate · ACES filmic</p>
        </div>
        <Suspense fallback={
          ...
          <ParkingScene
            occupiedA={zones.A.occupied} capacityA={zones.A.capacity}
            occupiedB={zones.B.occupied} capacityB={zones.B.capacity}
          />
        </Suspense>
      </div>
